...
├── notebooks/                    # Saved-output Kalman analysis record (kalman.ipynb)
...
The `notebooks/kalman.ipynb` notebook is a saved-output analysis record for inspecting the final three-state Kalman model results. It reads saved fit/PPC outputs from `results/` and does not refit models or run the Kalman filter. The notebook-local plotting helper lives in `notebooks/shared/`. The canonical exported figures remain in `figures/`, and the reproducible pipeline is the numbered script sequence in `code/`.
