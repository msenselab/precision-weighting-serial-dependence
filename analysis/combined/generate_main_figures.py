#!/usr/bin/env python3
"""
Generate manuscript figure source records from the public repository layout.
...
# Add repository shared module to path
...
def load_data():
    ...
    df1 = pd.read_pickle(base / "data" / "experiment1" / "E1.pkl")
    df1 = df1[df1["is_outlier"] == False].copy()

    df2 = pd.read_pickle(base / "data" / "experiment2" / "E2.pkl")
...
def compute_sdi_dataframes(df1, df2):
    ...
    # Exp 1 -- group SDI by coherence level
...
def plot_fig3_serial_dependence(df1, df2, df_sdi_1, df_sdi_2):
    ...
    Panel C: Exp 1 SDI boxplots by curCoherenceLevel.
    Panel D: Exp 2 SDI boxplots by SameSwitch.
    """
    ...
    # Panel C: Exp 1 SDI boxplots -- by Coherence
    ax = axes[1, 0]
    order_coh = ['Low', 'High']
    ...
    # Panel D: Exp 2 SDI boxplots -- by Same vs. Switch
    ax = axes[1, 1]
...
def plot_fig4_sanity_check(df1, df2):
    ...
    Panel A: Experiment 1 grouped by curCoherenceLevel.
    Panel B: Experiment 2 grouped by lag-specific Same/Switch
    ...
    # Panel A: Exp 1 -- grouped by current coherence for every lag
    ...
    # Panel B: Exp 2 -- Same/Switch recomputed for each lag.
...
def load_model_data(df1, df2):
    ...
    """
    base = ROOT
    MODEL_DIR = base / 'results' / 'kalman_model_fits'
    MODEL_CODE_DIR = base / 'models'
    ...
    try:
        ...
        from three_state_kalman import (
        ...
        print(f"Could not import three_state_kalman: {e}")
        return (False,) + (None,) * 12
...
def plot_fig8_trial_level_sd(df_model, results_df, best_exp1, best_exp2,
    ...
    Experiment-specific grouping applied:
        exp_num == 1 -> coherence grouping
        exp_num == 2   -> Same/Switch grouping
    """
